...
class DPRDistillTransform(nn.Module):
    ...
    def forward(self, batch: Dict[str, Any], stage: str ="train") -> Dict[str, Any]:
        """
        Combines the quesiton and the associated positive context embedding or
        ...
        for row in rows:
            ...
            if len(pos_ctx_vec) > 0:
                assert isinstance(pos_ctx_vec[0], list)

            #sample only one vector
            if stage == "train" and self.pos_ctx_sample:
                sampled_tensor = random.sample(
                    pos_ctx_vec, 1
                )
            else:
